...
mongoose.connect("mongodb://localhost:27017/InnovativeIdeaBox", {useNewUrlParser: true});
...
app.get("/idees_envoyees", function(req, res){
    if(req.isAuthenticated()){
        Idea.find({"idee": {$ne: null}, "user_id": req.user.id}, function(err, foundIdeas){
            if(err){
                console.log(err);
            } else{
                if(foundIdeas){
...
app.get("/voir_defi", function(req, res){
    if(req.isAuthenticated()){
        Idea.find({"idee": {$ne: null}, "type": "defi"}, function(err, foundIdeas){
            if(err){
                console.log(err);
            } else{
                if(foundIdeas){
                    ...
                    Defi.find({"defi": {$ne: null}}, function(err, foundDefi){
                        if(err){
                            ...
                            if(foundDefi){
                                console.log(foundDefi);
                                res.render("voir_defi", {ideas: foundIdeas, defi: foundDefi, statut: 2});
                            }
                        }
                    });               
                }
...
app.post("/register", function(req, res){
    ...
    User.register({mode: req.body.guard, username: req.body.username, email: req.body.email, name: req.body.name}, req.body.password, function(err, user){
        if(err){
            console.log(err);
